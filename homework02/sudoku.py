"""Генерация и решение головоломки судоку"""
...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    return [
        grid[r][c]
        for r in range(((pos[0] // 3) * 3), ((pos[0] // 3) * 3) + 3)
        for c in range(((pos[1] // 3) * 3), ((pos[1] // 3) * 3) + 3)
    ]
...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    for value in map(str, range(1, 10)):
        if value in find_possible_values(grid, (row, col)):
            grid[row][col] = value
            if solve(grid):
                return grid
            grid[row][col] = "."
